...
const EditTaskAttendeesTripView = (props) => {
  const { task } = props;
  const { tripId } = useParams();
  const dispatch = useDispatch();

  const [users, setUsers] = useState(props.trip.Users);
  ...
  }, [props.trip.Users, selectedUserId, userAccess]);

  function onSearchText(text, props) {
    let filtered;
    if (text && props.trip.Users.length) {
      filtered = props.trip.Users.filter(
        (user) =>
  ...
  const handleSubmit = (event) => {
    ...
    dispatch(
      updateTaskUser(selectedUserId, task.id, userAccess, "add", tripId)
    );
  ...
  const handleDelete = (event) => {
    ...
    dispatch(updateTaskUser(selectedUserId, task.id, role, "remove", tripId));
...
export default connect(mapState)(EditTaskAttendeesTripView);
